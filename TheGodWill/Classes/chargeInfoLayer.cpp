...
void chargeInfoLayer::addInfo()
{
    ...
    m_pBg = Sprite::createWithSpriteFrameName("shop_bg.png");
    m_pBg->setScale(2.0f);
    addChild(m_pBg);
...
void chargeInfoLayer::showGoodsInfo()
{
    ...
    m_scrollView = ScrollView::create(Size(bgSize.width * 0.93f, bgSize.height * 0.75f), scroll_layer);
    ...
    for(int i = 0; i < allSize; i++)
    {
        ...
        addStoneInfo("charge_bg.png", "charge_bg.png", m_stoneMenu, pos, goodId[i]);
    }
}
...
void chargeInfoLayer::addStoneInfo(const char* nor, const char* sel, Menu* parent, Vec2 pos, int tag)
{
    ...
    if(pInfo)
    {
    	...
    	Label* priceLabel = Label::createWithSystemFont(buf, "Arial", 34);
    	pItem->addChild(priceLabel);
    	priceLabel->setPosition(Vec2(normal->getContentSize().width * 0.5f, normal->getContentSize().height * 0.115f));
...
{
    Size bgSize = m_pBgLayer->getContentSize();
...
//    pCoin->setPosition(Vec2(coinBg->getBoundingBox().size.width * 0.2, coinBg->getContentSize().height * 0.5f));
    ...
    //魔法石
    Sprite* stoneBg = Sprite::createWithSpriteFrameName("charge_stoneBg.png");
    m_pBgLayer->addChild(stoneBg);
    stoneBg->setScale(0.85f);
    stoneBg->setPosition(Vec2(bgSize.width * 0.5, bgSize.height * 0.08f));
...
void chargeInfoLayer::btnCallBack(Ref *pSender)
{
    ...
    else
    {
        ...
        sprintf(buf, "%d", pInfo->chargeId);
        std::string productId = buf;
        ...
        log("商品信息%s", goodsInfo.c_str());
        
        //talkingData数据统计
        TDCCVirtualCurrency::onChargeRequest(Order.c_str(), pInfo->desc.c_str(), pInfo->price, "CNY", pInfo->num, GAME_VERSION);
        paymentClick(MY_PERSION->getUserInfo()->m_iUserId,  pInfo->num,  pInfo->price,  pInfo->price,
                     1 , productId, productName[tag - 1],  pInfo->desc, Order, goodsInfo);
    }
